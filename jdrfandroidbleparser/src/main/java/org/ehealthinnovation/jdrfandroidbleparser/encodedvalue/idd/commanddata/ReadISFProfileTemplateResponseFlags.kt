...
enum class ReadISFProfileTemplateResponseFlags constructor(val bit: Int) {
    ...
    companion object {
         ...
         */
        fun parseFlags(characteristicFlags: Int): EnumSet<ReadISFProfileTemplateResponseFlags> {
            val setFlags = EnumSet.noneOf(ReadISFProfileTemplateResponseFlags::class.java)
            ReadISFProfileTemplateResponseFlags.values().forEach {
                val flag = it.bit
        ...
        fun composeFlags(flags: EnumSet<ReadISFProfileTemplateResponseFlags>) :Int {
            var output = 0
            for (flag in flags){
                output += flag.bit
            }
