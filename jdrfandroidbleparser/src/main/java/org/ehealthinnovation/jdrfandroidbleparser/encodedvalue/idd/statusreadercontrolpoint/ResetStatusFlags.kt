...
enum class ResetStatusFlags constructor(val bit:Int) {
    ...
    companion object {
         ...
         */
        fun parseFlags(characteristicFlags: Int): EnumSet<ResetStatusFlags> {
            val setFlags = EnumSet.noneOf(ResetStatusFlags::class.java)
            ResetStatusFlags.values().forEach {
                val flag = it.bit
