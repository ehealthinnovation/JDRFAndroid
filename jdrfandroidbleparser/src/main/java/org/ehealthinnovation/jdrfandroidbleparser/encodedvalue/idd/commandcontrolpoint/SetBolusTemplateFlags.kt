...
enum class SetBolusTemplateFlags constructor(val bit: Int) {
    ...
    companion object {
         ...
         */
        fun parseFlags(characteristicFlags: Int): EnumSet<SetBolusTemplateFlags> {
            val setFlags = EnumSet.noneOf(SetBolusTemplateFlags::class.java)
            SetBolusTemplateFlags.values().forEach {
                val flag = it.bit
        ...
        fun composeFlags(flags: EnumSet<SetBolusTemplateFlags>) :Int {
            var output = 0
            for (flag in flags){
                output += flag.bit
            }
